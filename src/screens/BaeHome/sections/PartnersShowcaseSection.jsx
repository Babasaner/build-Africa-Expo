  ...
  return (
    <Card className="min-w-[166px] w-[166px] h-[100px] rounded-none border-[1px] border-[#d2d2d1] bg-white shadow-none transition-all duration-300 hover:border-[#36499b] hover:shadow-md">
      <CardContent className="flex h-full items-center justify-center p-4">
        <img alt={alt} src={src} className={`${imgClassName} transition-all duration-300`} />
      </CardContent>
    </Card>
  );
};
        ...
        {items.map((item, index) => (
          <div key={`${item.alt}-${index}`} className="snap-start">
            <LogoCard
              alt={item.alt}
              src={item.src}
              imgClassName={item.imgClassName || "w-[120px] h-auto object-contain"}
            />
          </div>
        ))}
      </div>
...
export const PartnersShowcaseSection = () => {
  ...
  useEffect(() => {
    ...
    client.fetch(query).then(data => {
      if (data && data.length > 0) {
        const grouped = {
          "Co-organisateurs": data.filter(p => p.category === 'co-organisateurs'),
  ...
  if (isLoading) {
    return <section className="relative w-full bg-[#E8E8E8] py-12 lg:py-[120px]"><div className="text-center">Chargement des partenaires...</div></section>;
  }
  ...
  return (
    <section className="relative w-full bg-[#E8E8E8] py-12 lg:py-[120px]">
      <div className="mx-auto flex w-full max-w-[1440px] flex-col items-start gap-10 px-5">
        <header>
          <h2 className="[font-family:'Tomorrow',Helvetica] text-2xl font-bold leading-[1.2] tracking-[1.08px] text-[#36499b] sm:text-3xl lg:text-4xl">
            PARTENAIRES
          </h2>
        </header>
